...
const Cart = () => {
    ...
    Cart.forEach((item, index) => {
      if (item.title) {
        ...
        if (item.bonus && item.bonus !== "0")
          message += `   • Bonus: ${item.bonus}\n`;
        message += `   • Price: ${item.price}\n`;
      } else {
        const name = item.key ? t(`food.${item.key}.name`) : "Unknown";
        message += `${index + 1}. ${name} x${item.amount} - ${
  ...
  const handleOrder = () => {
    ...
    if (!formData.fullName || !formData.city || !formData.phone)
      return alert(t("cart.fillDetails"));
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-6 pt-24 sm:pt-28 lg:pt-32 flex flex-col lg:flex-row gap-6">
      ...
      <div className="lg:w-2/3 w-full bg-white p-4 sm:p-6 rounded-lg shadow-md">
        ...
        {Cart.length > 0 ? (
          Cart.map((item, index) => (
            <div
              key={index}
              className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-center py-4 border-b text-gray-700"
            >
              {/* Product */}
              <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4">
                {item.image && (
                  <img
                    ...
                    alt={
                      item.title ||
                      (item.key ? t(`food.${item.key}.name`) : "Product")
                    }
              ...
              {/* Price */}
              <p className="text-lg font-medium text-left sm:text-center">
                {item.amount ? `DH${item.price}` : item.price}
              </p>

              {/* Total */}
              <p className="text-lg font-semibold text-left sm:text-center">
                {item.amount
                  ? `DH${item.price * item.amount}`
                  : item.price}
              </p>
            </div>
          ))
      ...
      {/* RIGHT */}
      <div className="lg:w-1/3 w-full bg-gray-100 p-4 sm:p-6 rounded-lg pb-4">
        ...
        <div className="flex justify-between mt-2">
          ...
          <span>DH{isNaN(Total) ? 0 : Total}</span>
        </div>
        <div className="flex justify-between mt-2">
